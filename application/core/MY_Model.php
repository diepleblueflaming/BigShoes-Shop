...
class MY_Model extends CI_Model {
    ...
    function create($data = array())
    {
        ...
        {
            // neu so ban ghi bi anh huong > 0 return true nguoc lai return false.
            return ($this->db->affected_rows() > 0);
        }else{
            $this->trigger_error("insert");
        }
    }

    // ham lay ra cau sql.
    public function get_sql($type){
        $action = "get_compiled_".$type;
        return $this->db->{$action}($this->table);
    }

    // ham lay ra loi.
    public function trigger_error($type){
        exit("MYSQL Query : ".$this->get_sql($type)."<br>".$this->db->_error_message());
    }
    ...
    function update_rule($where, $data)
    {
        ...
        // neu co loi thi bao loi va ngung chuong trinh.
        $this->trigger_error("update");
    }
    ...
    function get_list($input = array())
    {
        ...
        $this->get_list_set_input($input);
        //echo $this->get_sql("select");
        //thuc hien truy van du lieu

        // neu co choi dinh bang lay du lieu thi lay du lieu bang dc chi dinh.
        if(isset($input['join'])){
            $this->db->from($input['join'][0]);

            if(isset($input['join'][3])){
                $this->db->join($input['join'][1],$input['join'][2],$input['join'][3]);
            }else{
                $this->db->join($input['join'][1],$input['join'][2]);
            }
